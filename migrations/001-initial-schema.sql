...
CREATE TABLE IF NOT EXISTS companies (
  ...
  subscription_status TEXT DEFAULT 'inactive' CHECK (subscription_status IN ('active', 'inactive', 'pending', 'cancelled', 'overdue', 'trial')),
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_messages_content_dedup
ON messages(conversation_id, content, direction, sender_id, date_trunc('second', created_at))
WHERE length(content) > 0 AND external_id IS NULL;
...
-- Additional safety net for rapid duplicates (same content within 10 seconds)
CREATE UNIQUE INDEX IF NOT EXISTS idx_messages_rapid_dedup
ON messages(conversation_id, content, type, direction,
           floor(extract(epoch from created_at) / 10))
WHERE length(content) > 0 AND external_id IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_maintenance_mode_enabled ON maintenance_mode(enabled);
CREATE INDEX IF NOT EXISTS idx_maintenance_mode_created_at ON maintenance_mode(created_at DESC);
...
-- Insert initial maintenance mode record (disabled by default)
INSERT INTO maintenance_mode (enabled, message, created_at, updated_at)
VALUES (FALSE, 'System is under maintenance. Please try again later.', NOW(), NOW())
ON CONFLICT DO NOTHING;

-- Add comments for documentation
COMMENT ON TABLE system_updates IS 'Tracks system update history and progress';
COMMENT ON TABLE maintenance_mode IS 'Controls system maintenance mode settings';
...
COMMENT ON COLUMN maintenance_mode.enabled IS 'Whether maintenance mode is currently active';
COMMENT ON COLUMN maintenance_mode.message IS 'Message displayed to users during maintenance';
COMMENT ON COLUMN maintenance_mode.estimated_completion IS 'When maintenance is expected to complete';
COMMENT ON COLUMN maintenance_mode.bypass_code IS 'Secret code for admin access during maintenance';
COMMENT ON COLUMN maintenance_mode.enabled_by IS 'User ID who enabled maintenance mode';
